...
The model with no interaction was preferred as it provides a similar fit than 
...
dat_size <- rm_outlier_amp[rm_outlier_amp$Treatment == "Regular_singing", ]
...
$$SPL \sim + body\ size + (playback | individual) + (1 | song\ type)$$
...
random_slope_mod <- lmer(cal.spl ~ Treatment  + (Treatment|ID) + (1|songtype), data = playback_dat, REML = FALSE)
...
- The random slope model is significantly better than both the random intercept model (p = `r 
if (0.5*pchisq(d1[1], 0, lower.tail=FALSE) + 0.5*pchisq(d1[1], 1, lower.tail=FALSE) < 0.0001) "< 0.0001" else 0.5*pchisq(d1[1], 0, lower.tail=FALSE) + 0.5*pchisq(d1[1], 1, lower.tail=FALSE)`) and the null model (p = `r 
if (0.5*pchisq(d3[1], 0, lower.tail=FALSE) + 0.5*pchisq(d3[1], 1, lower.tail=FALSE) < 0.0001) "< 0.0001" else 0.5*pchisq(d3[1], 0, lower.tail=FALSE) + 0.5*pchisq(d3[1], 1, lower.tail=FALSE)`)


Note that data for individuals recorded in 2019 was not properly calibrated and has been centered around the population's average
...
$$SPL \sim + coordination + (coordination | individual) + (1 | song\ type)$$
1.Coordination as predictor and random intercept only:
$$SPL \sim + coordination + (1 | individual) + (1 | song\ type)$$
...
- The random slope model was not significantly better than the random intercept model (p = `r 
if (0.5*pchisq(d1[1], 0, lower.tail=FALSE) + 0.5*pchisq(d1[1], 1, lower.tail=FALSE) < 0.0001) "< 0.0001" else 0.5*pchisq(d1[1], 0, lower.tail=FALSE) + 0.5*pchisq(d1[1], 1, lower.tail=FALSE)`) nor the null model (p = `r 
if (0.5*pchisq(d3[1], 0, lower.tail=FALSE) + 0.5*pchisq(d3[1], 1, lower.tail=FALSE) < 0.0001) "< 0.0001" else 0.5*pchisq(d3[1], 0, lower.tail=FALSE) + 0.5*pchisq(d3[1], 1, lower.tail=FALSE)`)
...
$$SPL \sim + interaction + (1 | individual) + (1 | song\ type)$$
1. Interaction and interaction type as predictors:
$$SPL \sim + interaction + interaction\ type + (1 | individual) + (1 | song\ type)$$
...
dat_song_consistency_l <- lapply(unique(dat_song_consistency$ID.treatment), function(x){
  ...
  if(nrow(X) > 1){
    ...
    est <- selection_table(X,  path = "./data/raw/cuts", pb = FALSE, extended = TRUE, confirm.extended = FALSE)
    
        # measure cross-correlation
    xcrr <- try_na(cross_correlation(est, path = "./data/raw/cuts", pb = FALSE, wl = 150))  
...
Effect size posterior distribution
```{r Effect size posterior distribution song consistency, eval = TRUE, fig.height=10}
...
Effect size posterior distribution
```{r Effect size posterior distribution gaps, eval = TRUE, fig.height=10}
