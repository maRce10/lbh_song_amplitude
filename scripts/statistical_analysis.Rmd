...
- Change randome slope analysis with absolute differences in before and after treatments (both in interactions and coordinations)
-See if eliminating interaction videos allows a cleaner analysis (leave only calibrated songs)
...
```{r repetability plot for presentation}
...
# SPL vs condition and habitat structure
...
1. Vegetation density as predictor:
$$SPL \sim + vegetation\ density + (1 | individual) + (1 | song\ type)$$
1. Body size and vegetation density as predictors:
$$SPL \sim  + body\ size + vegetation\ density + (1 | individual) + (1 | song\ type)$$
1. Lifting power and vegetation density as predictors:
$$SPL \sim + lifting\ power + vegetation\ density + (1 | individual) + (1 | song\ type)$$

1. Lifting power and body size as predictors:
$$SPL \sim + lifting\ power + body\ size +  (1 | individual) + (1 | song\ type)$$
1. Lifting power, body size and vegetation density as predictors:
$$SPL \sim + lifting\ power + body\ size + vegetation\ density +  (1 | individual) + (1 | song\ type)$$

1. Interaction of lifting power and body size as predictor:
$$SPL \sim + lifting\ power * body\ size +  (1 | individual) + (1 | song\ type)$$

1. Interaction of lifting power, body size and vegetation density as predictor:
$$SPL \sim + lifting\ power * body\ size * vegetation\ density +  (1 | individual) + (1 | song\ type)$$
1. Null model with no predictor (intercept only):
$$SPL \sim + 1 + (1 | individual) + (1 | song\ type)$$
...
```{r SPL vs body condition and vegetation models, eval = FALSE}
...
agg_dat_agg <- aggregate(cal.spl ~ inter_treatment + inter_type, data = dat_aggresive_inter, mean)
agg_dat_agg$sd <- aggregate(cal.spl ~ inter_treatment + inter_type, data = dat_aggresive_inter, sd)$cal.spl

agg_dat_agg$inter_treatment <- factor(substr(agg_dat_agg$inter_treatment, 3, 100), levels = substr(agg_dat_agg$inter_treatment, 3, 100)[1:2])

ggplot(agg_dat_agg, aes(x = inter_treatment, y = cal.spl, color = inter_treatment)) +
  geom_point(size = 4, show.legend = FALSE) +
   geom_errorbar(width=.3, aes(ymin = cal.spl - sd, ymax = cal.spl + sd), show.legend = FALSE, lwd = 2) +
  labs(x = "Period", y = "Sound pressure level (dB)") +
  scale_color_viridis_d(begin = 0.1, end = 0.8, alpha = 1) +
  facet_wrap(~ inter_type, nrow = 1, scales = "free_y", labeller =  labeller(inter_type = 
    c("1_Perch_interaction" = "Perch interaction",
      "2_Chase" = "Chase"))) +
  theme_classic(base_size = 30)


# ggsave("./output/interaction_vs_SPL.jpeg", width = 11, height = 5.8)
...
```{r dynamic spectrogram, eval = FALSE}

wv <- readWave("./data/raw/cuts/397_SUR_03-Feb_2021_08.06_10.57_A56-11.wav")

spectro(cutw(wv, from = 1.2,to =  1.6, output = "Wave"), wl = 00, scale = FALSE, ovlp = 95, flim = c(1, 10))

scrolling_spectro(wave = wv, wl = 300, 
              t.display = 1.7, pal = viridis,  ovlp = 90, 
              grid = FALSE, flim = c(1, 9), colbg = "black", 
              width = 1000, height = 500,  collevels = seq(-50, 0, 5), 
              res = 120, file.name = "397_SUR_03-Feb_2021_08.06_10.57_A56-11_v2.mp4")

scrolling_spectro(wave = wv, wl = 300, speed = 0.5,
              t.display = 1.7, pal = viridis,  ovlp = 90, 
              grid = FALSE, flim = c(1, 9), colbg = "black", 
              width = 1000, height = 500,  collevels = seq(-50, 0, 5), 
              res = 120, file.name = "397_SUR_03-Feb_2021_08.06_10.57_A56-11_v3.mp4")

scrolling_spectro(wave = wv, wl = 300, 
    t.display = 1.7, ovlp = 90, pal = magma, 
    grid = FALSE, flim = c(1, 10), width = 1000, 
    height = 500, res = 120, collevels = seq(-40, 0, 5), 
    file.name = "397_SUR_03-Feb_2021_08.06_10.57_A56-11_v4.mp4", colbg = "black", 
...
scrolling_spectro(wave = Phae.long4, wl = 300, 
    ...
    file.name = "no_axis_short.mp4", colbg = "black", 
